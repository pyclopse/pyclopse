...
**Files:** `pyclawops/self/loader.py`, `pyclawops/self/knowledge/`,
`pyclawops/api/routes/self_.py`
**Served via:** pyclawops MCP server (port 8081) — no separate server

The self-knowledge system gives agents access to pyclawops's own documentation
...
| `self_source` | `(module: str) → str` | Read pyclawops source with line numbers |
...
# Read source code (path relative to pyclawops package)
...
Topics live in `pyclawops/self/knowledge/` and are served by `DocLoader`.
...
No separate config needed. The tools are part of the pyclawops MCP server (8081)
and available to any agent that has `pyclawops` in its `mcpServers` — which is
...
agents:
  my_agent:
    useFastagent: true
    mcpServers:
      - pyclawops      # ← self_topics/self_read/self_source are included here
      - fetch
      - time
      - filesystem
```
