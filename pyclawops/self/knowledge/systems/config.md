...
**Files:** `pyclawops/config/schema.py`, `pyclawops/config/loader.py`
...
agents:
  assistant:
    ...
    model: sonnet
    useFastagent: true
    mcpServers: [pyclawops, self, fetch]
    tools:
      profile: full
...
| `${keychain:Entry Name}` | macOS Keychain (service = `pyclawops`) |
| `${file:~/.secret}` | File contents (trimmed) |
| `${provider:name}` | Named secrets provider |

Resolution happens in `pyclawops/config/loader.py` via `SecretsManager`.
...
loader = ConfigLoader("~/.pyclawops/config.yaml")
...
```python
from pyclawops.config.schema import ExecApprovalsConfig, AgentConfig
