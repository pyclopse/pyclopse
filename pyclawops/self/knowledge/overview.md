...
pyclawops is a modular AI agent gateway. It connects LLM agents (backed by
...
```
1. load config
2. start pyclawops MCP server  (FastMCP on 8081) ← FastAgent connects to this; includes self-knowledge tools
...
pyclawops runs three HTTP servers in the same asyncio event loop:

| Server | Port | Library | Purpose |
|--------|------|---------|---------|
| pyclawops MCP | 8081 | FastMCP | Tool server for FastAgent (incl. self-knowledge tools) |
...
| File | Role |
|------|------|
| `pyclawops/core/gateway.py` | Main orchestrator: channels, sessions, agents, jobs, server lifecycle |
| `pyclawops/core/agent.py` | Agent dataclass + AgentManager; session runner cache |
| `pyclawops/agents/runner.py` | AgentRunner wraps FastAgent; `run_stream()` yields `(text, is_reasoning)` |
| `pyclawops/core/session.py` | Session (metadata only) + SessionManager; reaper; active_session pointer |
| `pyclawops/core/commands.py` | CommandRegistry: `/help /reset /status /model /job /skills /skill` |
| `pyclawops/core/queue.py` | SessionMessageQueue with 7 modes |
| `pyclawops/tools/server.py` | FastMCP server exposing all built-in tools (port 8081) |
| `pyclawops/self/server.py` | FastMCP self-knowledge server (port 8082) |
| `pyclawops/api/app.py` | FastAPI REST API (port 8080) |
| `pyclawops/jobs/scheduler.py` | Cron/interval/one-shot scheduler with notify_callback |
| `pyclawops/config/schema.py` | Pydantic config schema — all fields use validation_alias for camelCase |
| `pyclawops/config/loader.py` | Loads ~/.pyclawops/config.yaml; resolves ${source:id} inline secrets |
| `pyclawops/__main__.py` | CLI entry point; startup orchestration |
...
├── logs/
│   └── pyclawops.log            ← gateway log (daily rotation)
├── agents/
│   └── {agent_id}/
...
pyclawops looks for config in order:
1. `--config` CLI flag
2. `~/.pyclawops/config.yaml`
