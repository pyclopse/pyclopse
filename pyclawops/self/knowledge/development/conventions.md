...
Code and architecture patterns used throughout pyclawops. Follow these when adding
...
- Never hardcode paths — use `Path("~/.pyclawops/...").expanduser()`
...
- Never import from `pyclawops.core.gateway` inside a tool — go through the REST API
...
- All data under `~/.pyclawops/` — never write elsewhere without config override

---

## Logging

- Logger names follow the module hierarchy: `pyclawops.core.gateway`,
  `pyclawops.agent.{agent_id}`, `pyclawops.self`, etc.
- Per-agent loggers: `logging.getLogger(f"pyclawops.agent.{agent_id}")`
...
- No circular imports: `pyclawops.api.routes.*` imports from `pyclawops.api.app`
  using a deferred `_get_gateway()` function, not a module-level import
- `pyclawops.tools.server` should not import from `pyclawops.core.gateway` — use REST
