...
from pyclawops.secrets.models import SecretsConfig
from pyclawops.memory.vault.config import VaultConfig as _VaultConfig
...
class DockerSandboxConfig(BaseModel):
    ...
    image: str = "pyclawops-sandbox:latest"
    network: str = "none"
...
class AuditConfig(BaseModel):
    ...
    enabled: bool = True
    log_file: str = "~/.pyclawops/logs/audit.log"
...
class AgentConfig(BaseModel):
    ...
    # Leave unset if unknown; pyclawops will estimate from history file size.
    ...
    # Skills here override same-named global skills. Searched after ~/.pyclawops/skills/
    # and ~/.pyclawops/agents/{name}/skills/ but before any gateway.skills_dirs paths.
...
class PluginType(str, Enum):
    """Types of plugins supported by pyclawops."""
...
class PluginsConfig(BaseModel):
    ...
    # Entry-point plugins (pyclawops.channels group) are always discovered
...
class ChromeDevtoolsMcpConfig(BaseModel):
    ...
    Then add "chrome-devtools" to your agent::
        ...
        agents:
          assistant:
            mcpServers: [pyclawops, fetch, chrome-devtools]
    """
    enabled: bool = False
...
class GatewayConfig(BaseModel):
    ...
    # Additional skill search directories (on top of ~/.pyclawops/skills/)
    skills_dirs: List[str] = Field(
        default_factory=list,
        validation_alias=AliasChoices("skills_dirs", "skillsDirs"),
    )
...
class Config(BaseModel):
    ...
    Loaded from ``~/.pyclawops/config.yaml`` (or ``--config`` path).  All top-level
    ...
    # All pyclawops timestamps use this zone.  Omit to use the system local timezone.
