...
# "pyclawops" provides all built-in tools: bash, memory, todos, sessions, etc.
_DEFAULT_SERVERS: List[str] = ["pyclawops"]

# Built-in server names that pyclawops knows how to configure programmatically.
_BUILTIN_SERVERS = frozenset(["pyclawops", "fetch", "time", "filesystem", "chrome-devtools"])
...
class AgentRunner:
    ...
    Attributes:
        ...
        pyclawops_config (Optional[Any]): Full PyClawOps config object for building FA settings.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            servers (Optional[List[str]]): MCP server names. Defaults to ``["pyclawops"]``.
            ...
            pyclawops_config (Optional[Any]): Full PyClawOps config object for building FA Settings. Defaults to None.
        """
        ...
        # Optional API key / base URL (from pyclawops.yaml providers.minimax)
        self.api_key = api_key
        self.base_url = base_url
        # The agent name sent as X-Agent-Name header to the pyclawops MCP server.
        ...
        # Full PyClawOps config — used to build FastAgent Settings programmatically
        self.pyclawops_config: Optional[Any] = pyclawops_config
    ...
    def _build_fa_settings(self) -> Any:
        """Build a FastAgent Settings object directly from pyclawops config.
        ...
        PyClawOps config object.  Unknown server names (not in ``_BUILTIN_SERVERS``)
        are skipped with a warning.

        Returns:
            Any: A constructed ``fast_agent.config.Settings`` instance with MCP
            server definitions and provider credentials populated from pyclawops config.
        """
        ...
        for name in (self._all_servers() or ["pyclawops"]):
            if name == "pyclawops":
                servers["pyclawops"] = MCPServerSettings(
                ...
                logger.warning(
                    ...
                    "define it under pyclawops config to use custom servers"
                )
        ...
        if cfg is not None:
            ...
            # No pyclawops_config — fall back to individual api_key/base_url fields
            provider_kwargs["generic"] = GenericSettings(
                api_key=self.api_key,
                base_url=self.base_url,
            )
    ...
    async def initialize(self) -> None:
        ...
        pyclawops config (replacing any on-disk ``fastagent.config.yaml``), registers
    ...
    async def run(self, prompt: str) -> str:
        ...
        _agent_logger = logging.getLogger(f"pyclawops.agent.{self.owner_name}")
        _p = self._log_prefix
        ...
        async with self._run_lock:
            ...
            try:
                ...
                from pyclawops.core.concurrency import get_manager
        ...
        try:
            ...
            if self._slash_handler is None:
                ...
                self._slash_handler = SlashCommandHandler(
                    session_id=self.session_id or f"pyclawops-{self.agent_name}",
                    instance=instance,
