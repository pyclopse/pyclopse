...
You are running inside **pyclawops**. Your workspace is at `~/.pyclawops/agents/<name>/`.
...
Tools are provided via MCP. Key built-in tools (via the `pyclawops` MCP server):
- `bash` — run shell commands
- `memory_*` — search and store memories
- `jobs_*` — manage scheduled jobs
- `config_*` — read/update gateway config

Skills live at `~/.pyclawops/skills/` (global) and `~/.pyclawops/agents/<name>/skills/` (yours).
