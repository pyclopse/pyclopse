"""Slash command registry for pyclawops gateway."""
...
logger = logging.getLogger("pyclawops.commands")
...
class CommandRegistry:
    """Registry of slash commands available in pyclawops.
...
def register_builtin_commands(registry: CommandRegistry, gateway: Any) -> None:
    ...
    async def cmd_reset(args: str, ctx: CommandContext) -> str:
        ...
        if ctx.session.history_dir and ctx.session.history_dir.exists():
            ...
            from pyclawops.utils.time import now as _now
    ...
    async def cmd_start(args: str, ctx: CommandContext) -> str:
        return (
            "👋 Hello! I'm your pyclawops assistant.\n\n"
            + registry.help_text()
        )
    ...
    async def cmd_config(args: str, ctx: CommandContext) -> str:
        ...
        if action == "set":
            ...
            return "⚠️ Live config edits are not yet supported via command. Edit ~/.pyclawops/config/pyclawops.yaml and use /reload."
    ...
    async def cmd_reboot(args: str, ctx: CommandContext) -> str:
        """Hard-restart the pyclawops process (picks up code + config changes)."""
        import asyncio
        import os
        import sys
        ...
        async def _do_reboot():
            ...
            try:
                ...
                pass
            # When run via `python -m pyclawops`, sys.argv[0] is __main__.py.
            # Re-executing that path directly breaks relative imports, so we
            # rebuild the argv using -m instead.
            if sys.argv[0].endswith("__main__.py"):
                argv = [sys.executable, "-m", "pyclawops"] + sys.argv[1:]
            else:
                argv = [sys.executable] + sys.argv
            os.execv(sys.executable, argv)

        asyncio.create_task(_do_reboot())
        return "🔄 Rebooting pyclawops… (back in a few seconds)"
    ...
    async def cmd_queue(args: str, ctx: CommandContext) -> str:
        ...
        """
        from pyclawops.config.schema import QueueConfig, QueueMode, DropPolicy
    ...
    async def cmd_memories(args: str, ctx: CommandContext) -> str:
        ...
        try:
            ...
            from pyclawops.memory.vault.retrieval import FallbackSearchBackend  # noqa: F401
