"""Centralised "current time" helper for pyclawops.

All timestamps in pyclawops use timezone-naive datetimes expressed in a single,
...
def configure_timezone(tz_name: Optional[str]) -> None:
    ...
    if not tz_name:
        _configured_tz = None
        logger.debug("pyclawops timezone: system local")
        return
    try:
        from zoneinfo import ZoneInfo
        _configured_tz = ZoneInfo(tz_name)
        logger.debug(f"pyclawops timezone: {tz_name}")
...
def now() -> datetime:
    ...
    This is the single source of truth for all pyclawops timestamps.
