...
from pyclawops.reflect import reflect_system as _reflect_system


@_reflect_system("mcp-server")
class _PyclawMCPServer:
    """FastMCP tool server — exposes all pyclawops-native tools to FastAgent.
...
# Logs every tool call to the per-agent logger (pyclawops.agent.<name>).
# ---------------------------------------------------------------------------
...
class _ToolLoggingMiddleware(Middleware):
    ...
    async def on_call_tool(self, context: MiddlewareContext, call_next) -> Any:
        ...
        alogger = logging.getLogger(f"pyclawops.agent.{agent_name}")
        prefix = (
...
async def bash(
    ...
    # command's working directory rather than inheriting pyclawops's venv.
...
async def sessions_list() -> str:
    """List all active pyclawops sessions with their agent, channel, and status.
    ...
    try:
        ...
        loader = ConfigLoader()
        sessions_dir = Path("~/.pyclawops/sessions").expanduser()
...
def _agent_memory_service(agent_name: Optional[str] = None):
    ...
    Args:
        agent_name (Optional[str]): Agent name used to resolve the memory directory
            (``~/.pyclawops/agents/{agent_name}``).  Falls back to the
    ...
    config_dir = os.environ.get("PYCLAW_CONFIG_DIR", "~/.pyclawops")
    ...
    try:
        from pyclawops.memory.service import get_memory_service, MemoryService
...
async def session_status() -> str:
    ...
    try:
        import json
        status_file = Path("~/.pyclawops/status.json").expanduser()
...
async def sessions_send(
    ...
    """
    Send a message directly into another pyclawops agent's active session and get a response.
    ...
    try:
        ...
        if not agent_id:
            cfg_path = Path("~/.pyclawops/config/pyclawops.yaml").expanduser()
            if cfg_path.exists():
...
async def agents_list() -> str:
    """List all configured pyclawops agents with their model and status."""
    try:
        cfg_path = Path("~/.pyclawops/config/pyclawops.yaml").expanduser()
        if not cfg_path.exists():
            return "No pyclawops config found."
...
def _discover_all_skills(caller: Optional[str] = None) -> list:
    ...
    from pyclawops.skills.registry import discover_skills, get_skill_dirs, _parse_skill_dir
    import os as _os
    from pathlib import Path
    config_dir = Path(_os.path.expanduser("~/.pyclawops"))
...
def skills_list(all_agents: bool = False) -> str:
    ...
    """
    try:
        from pyclawops.skills.registry import discover_skills
        caller = _get_caller_agent()
        ...
        if not skills:
            return "No skills installed. Add skill directories to ~/.pyclawops/skills/."
...
def skill_read(name: str) -> str:
    ...
    """
    try:
        from pyclawops.skills.registry import find_skill
        agent = _get_caller_agent()
...
# ---------------------------------------------------------------------------
# Config — CRUD on pyclawops.yaml, validate, reload
# ---------------------------------------------------------------------------

def _find_config_path() -> Optional[Path]:
    """Locate the active pyclawops config file (same search order as ConfigLoader)."""
    from pyclawops.config.loader import DEFAULT_CONFIG_PATHS, expand_path
...
def config_get() -> str:
    """
    Return the current pyclawops configuration (sensitive fields redacted).
    ...
    Args:
        ...
        value: New value as JSON or a plain string.
               ...
               Examples:
                 ...
                 "true" / "false"            → boolean
                 "2048"                      → integer
                 '["pyclawops","fetch"]'        → list
    """
    try:
        cfg_path = _find_config_path()
        if cfg_path is None:
            return "[ERROR] No pyclawops config file found."
...
def config_schema(section: str = "") -> str:
    """
    Return the JSON schema for the pyclawops configuration (or a specific section).
    ...
    if cfg_path is not None:
        try:
            ...
            log_file = (
                raw.get("security", {})
                   .get("audit", {})
                   .get("log_file", "~/.pyclawops/logs/audit.log")
            )
            return Path(os.path.expanduser(log_file))
        except Exception:
            pass
    return Path(os.path.expanduser("~/.pyclawops/logs/audit.log"))
...
async def workflow_chain(
    ...
    try:
        from pyclawops.workflows.chain import run_chain
...
async def workflow_parallel(
    ...
    try:
        from pyclawops.workflows.parallel import run_parallel
...
async def secrets_list() -> str:
    """
    List all secret names registered in the pyclawops secrets registry.
...
async def secret_get(name: str) -> str:
    """
    Retrieve a named secret value from the pyclawops secrets registry.

    The secret must be registered under 'secrets:' in pyclawops.yaml.
...
def reflect(category: str = "", name: str = "") -> str:
    """Query pyclawops's live architecture reflection registry.

    Returns structured documentation about pyclawops's internals, derived from
    ...
    """
    from pyclawops.reflect import query as _reflect_query
...
def reflect_source(module: str) -> str:
    ...
    Returns the source of any file within the pyclawops package, with line numbers
    prepended to each line. Only paths within the pyclawops package are accessible.

    Use this for fine-grained implementation detail. Pair with reflect() for
    conceptual understanding.

    Args:
        module: Path relative to the pyclawops package root, using forward slashes.
...
async def vault_search(query: str, limit: int = 15) -> str:
    ...
    try:
        from pyclawops.memory.vault.retrieval import infer_profile
...
def vault_facts_list(type_filter: str = "", state_filter: str = "all") -> str:
    ...
    try:
        from pyclawops.memory.vault.models import VaultFactState
        states = None
        types = None
...
async def vault_bulk_ingest(
    ...
    try:
        ...
        from pyclawops.memory.vault.bulk import BulkIngestor
...
async def vault_fact_store(
    ...
    try:
        from pyclawops.memory.vault.models import VaultFact, VaultFactState
        fact = VaultFact(
            claim=claim,
            type=type,
