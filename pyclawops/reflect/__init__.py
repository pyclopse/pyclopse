...
MCP tool queries it so agents can explore pyclawops's own architecture live.
...
class _ReflectionSystem:
    """Live architecture reflection system for pyclawops.

    Decorators annotate classes and functions at import time so agents can
    explore pyclawops's structure without reading static documentation.

    **Decorators:**

    - ``@reflect_system("name")`` — a major pyclawops subsystem (class or sentinel)
    ...
    **MCP tools (in pyclawops/tools/server.py):**
    ...
    **Query API:**
        ...
        reflect()                              → pyclawops.__doc__ overview
