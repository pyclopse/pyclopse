"""Source file reader for the pyclawops reflection system.

Reads pyclawops source files with line numbers, for use by the
...
# pyclawops package root — resolved relative to this file so it works in both
...
def source_file(module: str) -> str:
    ...
    *module* is a path relative to the pyclawops package root, e.g.
    ...
    pyclawops package are accessible; directory traversal is rejected.

    Returns the file contents with ``lineno\\t`` prefixes, or a descriptive
    error/not-found string.

    Args:
        module: Forward-slash path relative to the pyclawops package root.
