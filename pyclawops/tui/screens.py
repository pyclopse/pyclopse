...
# Debug log file path
DEBUG_LOG = Path("/tmp/pyclawops_tui_debug.log")
...
class ChatScreen(Screen):
    ...
    def _send_message(self, message: str) -> None:
        ...
        if self.gateway and self._current_agent_id:
            ...
            self._append_chat("")
            self._append_chat(
                f"[yellow]PyClawOps:[/yellow] Gateway running! Configure agents in config to enable chat."
            )
        else:
            # No gateway - demo mode
            self._append_chat("")
            self._append_chat(
                f"[yellow]PyClawOps:[/yellow] Gateway not connected. Start with --tui flag."
            )
    ...
    async def _process_message(self, message: str) -> None:
        ...
        try:
            ...
            try:
                ...
                from pyclawops.agents.runner import strip_thinking_tags, _THINKING_RE
                import re as _re
...
class LogsScreen(Screen):
    ...
    def on_mount(self) -> None:
        ...
        self._log_viewer.write("[bold]PyClawOps Log Viewer[/bold]")
