"""Gateway Dashboard TUI for pyclawops.
...
class SystemPromptView(Vertical):
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            text = build_system_prompt(agent_name=self._agent_id, config_dir="~/.pyclawops")
        except Exception as e:
...
class FileBrowserView(Vertical):
    """File browser + viewer/editor for ~/.pyclawops/agents/{id}/"""
    ...
    def __init__(self, gateway: Any, **kwargs: Any) -> None:
        ...
        self._agents_dir = Path("~/.pyclawops/agents").expanduser()
...
class SkillsView(Vertical):
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            if am:
                agent = am.agents.get(self._agent_id)
                if agent:
                    pc = getattr(agent, "pyclawops_config", None)
    ...
    def _load_body(self, skill_name: str) -> None:
        ...
        try:
            ...
            skill = find_skill(skill_name, agent_name=self._agent_id, config_dir="~/.pyclawops")
            if skill:
...
class RunHistoryView(Vertical):
    """Shows run history for a job from ~/.pyclawops/agents/{id}/runs/ JSONL."""
    ...
    def _load(self) -> None:
        ...
        runs_file = (
            Path("~/.pyclawops/agents").expanduser()
            / self._agent_id
            / "runs"
...
class AgentLogView(Vertical):
    """Live tail of ~/.pyclawops/agents/{id}/logs/agent.log"""
    ...
    def refresh_for_agent(self, agent_id: str) -> None:
        ...
        self._agent_id = agent_id
        self._log_path = (
            Path("~/.pyclawops/agents").expanduser() / agent_id / "logs" / "agent.log"
        )
        self._last_size = 0
        self._do_load()
...
def _build_a2a_card(agent_id: str, gateway: Any) -> dict:
    ...
    # Pyclaw version
    try:
        from pyclawops import __version__ as _pyclawops_ver
        version = _pyclawops_ver
    except Exception:
        version = "0.0.0"
    ...
    if not description:
        description = f"pyclawops agent: {agent_id}"
    ...
    # Skills — convert pyclawops skills to A2A AgentSkill objects
    a2a_skills: list = []
    try:
        from pyclawops.skills.registry import discover_skills
...
class TracesView(Vertical):
    ...
    def _load_spans(self) -> None:
        from pyclawops.core import otel_store
...
class GatewayDashboard(App):
    """Unified pyclawops gateway dashboard."""

    TITLE = "PyClawOps Gateway"
    SUB_TITLE = "Dashboard"
    ...
    def _get_yank_text(self) -> str:
        ...
        try:
            if view == "agentcard":
                ...
                from pyclawops.core.prompt_builder import build_system_prompt
                return build_system_prompt(agent_name=agent, config_dir="~/.pyclawops")
            elif view == "files":
                fb = self.query_one(FileBrowserView)
                ...
                try:
                    ...
                    from pyclawops.skills.registry import find_skill
                    skill = find_skill(skill_name, agent_name=agent, config_dir="~/.pyclawops")
                    if skill:
                ...
                log_path = (
                    Path("~/.pyclawops/agents").expanduser() / agent / "logs" / "agent.log"
                )
                if log_path.exists():
