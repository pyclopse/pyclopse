"""CLI entry point for pyclawops.

Usage:
    python -m pyclawops              # Run gateway + dashboard TUI
    python -m pyclawops --help      # Show help
    python -m pyclawops init        # Create default config
    python -m pyclawops validate     # Validate config
    python -m pyclawops run          # Run with dashboard TUI (default)
    python -m pyclawops run --headless  # Run without TUI (stdout only)
"""
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="pyclawops",
        description="pyclawops - Python Gateway",
    ...
    parser.add_argument(
        ...
        "-v",
        action="version",
        version=f"pyclawops {__version__}",
    )
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update pyclawops to the latest release",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall pyclawops",
    )
    uninstall_parser.add_argument(
        "--purge",
        action="store_true",
        help="Also remove ~/.pyclawops/ config and data without prompting",
    )

    # import-openclaw command
    import_parser = subparsers.add_parser(
        "import-openclaw",
        help="Import OpenClaw session history into pyclawops",
    )
    ...
    import_parser.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="Path to pyclawops data directory (default: ~/.pyclawops)",
    )
...
class _ExcludeAgentDetailFilter(logging.Filter):
    """Block pyclawops.agent.* INFO/DEBUG records from the main pyclawops.log.
    ...
    def filter(self, record: logging.LogRecord) -> bool:  # type: ignore[override]
        if record.name.startswith("pyclawops.agent.") and record.levelno < logging.WARNING:
            return False
        return True


def setup_logging(config, debug: bool = False) -> None:
    """Configure root logger: console + daily-rotating file under ~/.pyclawops/logs/."""
    ...
    # Daily rotating file handler → ~/.pyclawops/logs/pyclawops.log
    logs_dir = Path("~/.pyclawops/logs").expanduser()
    logs_dir.mkdir(parents=True, exist_ok=True)
    log_file = logs_dir / "pyclawops.log"
    ...
    # Rename rotated files from "pyclawops.log.YYYY-MM-DD" → "pyclawops-YYYY-MM-DD.log"
    def _namer(default_name: str) -> str:
        base, _, suffix = default_name.rpartition(".")
        if suffix and len(suffix) == 10:  # YYYY-MM-DD
            return str(logs_dir / f"pyclawops-{suffix}.log")
        return default_name
...
def setup_agent_logging(agent_id: str, logs_dir: Path, retention_days: int) -> None:
    """Set up a daily-rotating per-agent log under ~/.pyclawops/agents/{agent_id}/logs/.

    All records emitted to logger ``pyclawops.agent.{agent_id}`` (and its children)
    ...
    agent_logger = logging.getLogger(f"pyclawops.agent.{agent_id}")
    ...
    # Do NOT set propagate=False so records also flow to root (console + pyclawops.log at WARNING+)
...
def _register_skill_providers(mcp_server, config) -> None:
    ...
    try:
        ...
        from pyclawops.skills.registry import get_skill_dirs
...
async def run_gateway(
    ...
    """Run the gateway + HTTP API server + pyclawops MCP server."""
    ...
    print(f"pyclawops v{__version__}")
    print(f"Starting pyclawops MCP server on {gw_host}:{mcp_port}")
    ...
    from .tools.server import mcp as pyclawops_mcp
    _register_skill_providers(pyclawops_mcp, config)
    ...
    _agents_logs_dir = Path("~/.pyclawops/agents").expanduser()
...
def cmd_validate(args):
    ...
    config_path = args.config or "~/.pyclawops/config.yaml"
...
def cmd_secret(args):
    ...
    if args.secret_command != "get":
        print("Usage: pyclawops secret get <name>")
        sys.exit(1)

    from pyclawops.secrets.manager import SecretsManager, ResolutionError
    from pyclawops.config.loader import load_secrets_registry, find_config_file, expand_path
...
def cmd_update(args):
    ...
    if args.beta:
        ...
        print(f"Installing pyclawops {label}...")
        try:
            subprocess.run(["uv", "tool", "install", "--reinstall", url], check=True)
            print(f"✓ pyclawops updated to {label}")
            print("  Your config and data in ~/.pyclawops/ are untouched.")
        ...
        print("Upgrading pyclawops to the latest release...")
        try:
            subprocess.run(["uv", "tool", "upgrade", "pyclawops"], check=True)
            print("✓ pyclawops upgraded.")
            print("  Your config and data in ~/.pyclawops/ are untouched.")
        except subprocess.CalledProcessError:
            print("✗ Upgrade failed. Is pyclawops installed via 'uv tool'?")
            sys.exit(1)
...
def cmd_uninstall(args):
    ...
    pyclawops_dir = Path("~/.pyclawops").expanduser()
    remove_data = False

    if args.purge:
        remove_data = True
    elif pyclawops_dir.exists():
        try:
            answer = input(f"Remove {pyclawops_dir} (config, sessions, memory)? [y/N] ").strip().lower()
    ...
    try:
        subprocess.run(["uv", "tool", "uninstall", "pyclawops"], check=True)
    except subprocess.CalledProcessError:
        print("✗ Uninstall failed — is pyclawops installed via 'uv tool'?")
        sys.exit(1)

    if remove_data and pyclawops_dir.exists():
        shutil.rmtree(pyclawops_dir)
        print(f"✓ Removed {pyclawops_dir}")
    elif pyclawops_dir.exists():
        print(f"  Config and data kept at {pyclawops_dir}")

    print("✓ pyclawops uninstalled.")
