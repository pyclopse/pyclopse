...
1. **Entry points** — installed packages declare a ``pyclawops.channels`` entry
...
2. **Explicit config** — the ``plugins.channels`` list in ``pyclawops.yaml``
...
:class:`~pyclawops.channels.plugin.ChannelPlugin`.
"""
...
logger = logging.getLogger("pyclawops.channels")

_ENTRY_POINT_GROUP = "pyclawops.channels"
...
def discover_entry_points() -> List[ChannelPlugin]:
    ...
    ``pyclawops.channels`` will be loaded. Errors are logged and skipped so
