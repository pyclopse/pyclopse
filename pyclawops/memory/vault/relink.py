...
Usage::
    ...
    uv run python -m pyclawops.memory.vault.relink <agent_name> [options]

    # Dry run (show what would change, no writes)
    uv run python -m pyclawops.memory.vault.relink niggy --dry-run

    # Set custom similarity threshold (default 3.0)
    uv run python -m pyclawops.memory.vault.relink niggy --threshold 2.0

    # Multiple agents
    uv run python -m pyclawops.memory.vault.relink niggy ritchie via

    # Process all agents found under ~/.pyclawops/agents/
    uv run python -m pyclawops.memory.vault.relink --all
...
logger = logging.getLogger("pyclawops.vault.relink")


def _get_agents_dir() -> Path:
    return Path.home() / ".pyclawops" / "agents"
