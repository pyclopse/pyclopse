...
Each agent gets its own directory under ``~/.pyclawops/agents/{agent_name}/``:
...
``pyclawops.core.prompt_builder.build_system_prompt``.
...
Vector index
------------
When an :class:`~pyclawops.memory.embeddings.EmbeddingBackend` is supplied, each
...
logger = logging.getLogger("pyclawops.memory")
    ...
    def __init__(
        ...
        Args:
            base_dir (str): The agent's root directory, e.g.
                ``~/.pyclawops/agents/myagent``.  The ``memory/`` subdirectory
