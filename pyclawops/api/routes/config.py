...
logger = logging.getLogger("pyclawops.api.config")
...
async def list_secrets():
    ...
    """
    from pyclawops.config.loader import load_secrets_registry
...
async def get_secret(name: str):
    """
    Resolve a named secret via the pyclawops secrets manager.

    The secret must be registered in ~/.pyclawops/secrets/secrets.yaml.
    """
    from pyclawops.config.loader import load_secrets_registry
