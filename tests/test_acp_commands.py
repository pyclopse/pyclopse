"""Tests for ACP slash command wiring in pyclopse.
...
def test_get_runner_returns_existing_runner():
    from pyclopse.core.commands import _get_runner, CommandContext
    runner = _make_runner()
...
async def test_model_reasoning_routes_to_acp():
    from pyclopse.core.commands import CommandContext, register_builtin_commands, CommandRegistry
    runner = _make_runner()
    gw, session, _ = _make_gateway(runner=runner)
...
async def test_acp_execute_uses_slash_handler():
    ...
    with patch("pyclopse.agents.runner.AgentRunner.acp_execute", new_callable=AsyncMock) as mock_exec:
...
def test_acp_config_defaults():
    from pyclopse.config.schema import AcpConfig
    cfg = AcpConfig()
...
async def test_status_no_args_returns_gateway_status():
    """Plain /status without args still shows the pyclopse gateway summary."""
    from pyclopse.core.commands import CommandContext, register_builtin_commands, CommandRegistry
    runner = _make_runner()
    gw, session, _ = _make_gateway(runner=runner)
