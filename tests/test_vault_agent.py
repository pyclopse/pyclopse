...
from pyclopse.memory.vault.agent import (
...
async def test_extract_from_conversation_uses_runner(mock_runner, registry):
    ...
    with patch("pyclopse.memory.vault.agent.FastAgentMemoryAgent._get_runner", return_value=mock_runner):
...
async def test_extract_passes_existing_facts_in_prompt(registry):
    ...
    agent = FastAgentMemoryAgent(model="generic.test-model")
    with patch("pyclopse.memory.vault.agent.FastAgentMemoryAgent._get_runner", return_value=mock_r):
