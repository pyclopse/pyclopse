"""Tests for the pyclaw skill system.
...
from pyclaw.skills.registry import (
    SkillInfo,
    discover_skills,
    find_skill,
    format_for_prompt,
...
@pytest.fixture
def pyclaw_home(skill_tree):
    """Temp pyclaw home directory with the skill tree."""
    return skill_tree
...
class TestSkillRegistry:
    ...
    def test_discover_finds_skills(self, pyclaw_home):
        skills = discover_skills(config_dir=str(pyclaw_home))
    ...
    def test_skill_info_fields(self, pyclaw_home):
        skills = discover_skills(config_dir=str(pyclaw_home))
    ...
    def test_skill_allowed_tools(self, pyclaw_home):
        skills = discover_skills(config_dir=str(pyclaw_home))
        fc = next(s for s in skills if s.name == "file-counter")
        assert fc.allowed_tools == ["bash"]

    def test_skill_dir_substitution(self, pyclaw_home):
        skills = discover_skills(config_dir=str(pyclaw_home))
        hw = next(s for s in skills if s.name == "hello-world")
    ...
    def test_find_skill_found(self, pyclaw_home):
        skill = find_skill("hello-world", config_dir=str(pyclaw_home))
        assert skill is not None
        assert skill.name == "hello-world"

    def test_find_skill_case_insensitive(self, pyclaw_home):
        skill = find_skill("Hello-World", config_dir=str(pyclaw_home))
        assert skill is not None

    def test_find_skill_not_found(self, pyclaw_home):
        assert find_skill("nonexistent-xyz", config_dir=str(pyclaw_home)) is None
...
class TestFormatForPrompt:
    ...
    def test_contains_skill_name(self, pyclaw_home):
        skills = discover_skills(config_dir=str(pyclaw_home))
        text = format_for_prompt(skills)
        assert "hello-world" in text
        assert "file-counter" in text

    def test_xml_structure(self, pyclaw_home):
        skills = discover_skills(config_dir=str(pyclaw_home))
        text = format_for_prompt(skills)
    ...
    def test_custom_read_tool_name(self, pyclaw_home):
        skills = discover_skills(config_dir=str(pyclaw_home))
...
async def _pyclaw_mcp_session(home_dir: str):
    env = {
        **os.environ,
        "HOME": home_dir,
...
async def test_mcp_skills_list_no_skills(tmp_path):
    ...
    params = await _pyclaw_mcp_session(str(tmp_path))
...
async def test_mcp_skills_list_with_skills(tmp_path):
    """skills_list finds and lists skills in HOME/.pyclaw/skills/."""
    pyclaw_dir = tmp_path / ".pyclaw" / "skills" / "demo-skill"
    pyclaw_dir.mkdir(parents=True)
    (pyclaw_dir / "SKILL.md").write_text(
...
async def test_mcp_skill_read(tmp_path):
    ...
    skill_dir = tmp_path / ".pyclaw" / "skills" / "reader-skill"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
...
class TestSkillCommands:
    ...
    def _make_ctx(self, pyclaw_home, agent_id="test"):
    ...
    async def test_cmd_skills_no_skills(self, tmp_path):
        ...
        with patch("pyclaw.skills.registry.get_skill_dirs", return_value=[]):
            result = await registry.dispatch("/skills", ctx)
        assert result is not None
        assert "No skills" in result or "skill" in result.lower()

    @pytest.mark.asyncio
    async def test_cmd_skills_lists_skills(self, pyclaw_home):
        """Test /skills command lists discovered skills."""
        from pyclaw.core.commands import CommandRegistry, register_builtin_commands, CommandContext
        from unittest.mock import MagicMock, patch
        ...
        with patch(
            "pyclaw.skills.registry.get_skill_dirs",
            return_value=[pyclaw_home / "skills"],
        ):
            result = await registry.dispatch("/skills", ctx)
    ...
    @pytest.mark.asyncio
    async def test_cmd_skill_no_args(self, pyclaw_home):
    ...
    @pytest.mark.asyncio
    async def test_cmd_skill_not_found(self, pyclaw_home):
