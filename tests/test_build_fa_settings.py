...
def _make_runner(servers=None, pyclaw_config=None, model="sonnet",
    ...
    r._log_prefix = "[main]"
    r.model = model
    r.servers = servers if servers is not None else ["pyclaw"]
    r.pyclaw_config = pyclaw_config
    r.api_key = api_key
    r.base_url = base_url
    return r
...
def test_pyclaw_server_url_from_mcp_port():
    runner = _make_runner(pyclaw_config=_make_pyclaw_config(mcp_port=9999))
    settings = runner._build_fa_settings()
    assert settings.mcp.servers["pyclaw"].url == "http://localhost:9999/mcp"


def test_pyclaw_server_default_port_without_config():
    runner = _make_runner(pyclaw_config=None)
    settings = runner._build_fa_settings()
    assert settings.mcp.servers["pyclaw"].url == "http://localhost:8081/mcp"


def test_pyclaw_server_headers_include_agent_name():
    runner = _make_runner(pyclaw_config=_make_pyclaw_config())
    settings = runner._build_fa_settings()
    assert settings.mcp.servers["pyclaw"].headers["x-agent-name"] == "main"


def test_pyclaw_server_headers_include_session_id():
    runner = _make_runner(pyclaw_config=_make_pyclaw_config(), session_id="sess-123abc")
    settings = runner._build_fa_settings()
    assert settings.mcp.servers["pyclaw"].headers["x-session-id"] == "sess-123abc"


def test_pyclaw_server_no_session_id_header_omitted():
    runner = _make_runner(pyclaw_config=_make_pyclaw_config(), session_id=None)
    settings = runner._build_fa_settings()
    assert "x-session-id" not in (settings.mcp.servers["pyclaw"].headers or {})


# ── standard servers ──────────────────────────────────────────────────────────

def test_fetch_server_defined_when_requested():
    runner = _make_runner(servers=["pyclaw", "fetch"], pyclaw_config=_make_pyclaw_config())
    settings = runner._build_fa_settings()
...
def test_fetch_server_not_defined_when_not_requested():
    runner = _make_runner(servers=["pyclaw"], pyclaw_config=_make_pyclaw_config())
    settings = runner._build_fa_settings()
...
def test_time_server_with_timezone():
    runner = _make_runner(
        servers=["pyclaw", "time"],
        pyclaw_config=_make_pyclaw_config(timezone="Europe/London"),
    )
    settings = runner._build_fa_settings()
...
def test_filesystem_server_defined_when_requested():
    runner = _make_runner(servers=["pyclaw", "filesystem"], pyclaw_config=_make_pyclaw_config())
    settings = runner._build_fa_settings()
...
def test_chrome_devtools_added_when_enabled_and_requested():
    runner = _make_runner(
        servers=["pyclaw", "chrome-devtools"],
        pyclaw_config=_make_pyclaw_config(chrome_enabled=True, chrome_auto_connect=True),
    )
    settings = runner._build_fa_settings()
...
def test_chrome_devtools_skipped_when_not_enabled():
    runner = _make_runner(
        servers=["pyclaw", "chrome-devtools"],
        pyclaw_config=_make_pyclaw_config(chrome_enabled=False),
    )
    settings = runner._build_fa_settings()
...
def test_chrome_devtools_browser_url_in_args():
    runner = _make_runner(
        servers=["pyclaw", "chrome-devtools"],
        pyclaw_config=_make_pyclaw_config(
            chrome_enabled=True,
...
def test_anthropic_key_in_settings():
    runner = _make_runner(pyclaw_config=_make_pyclaw_config(anthropic_key="sk-ant-abc"))
    settings = runner._build_fa_settings()
...
def test_openai_key_in_settings():
    runner = _make_runner(pyclaw_config=_make_pyclaw_config(openai_key="sk-oai-xyz"))
    settings = runner._build_fa_settings()
...
def test_google_key_in_settings():
    runner = _make_runner(pyclaw_config=_make_pyclaw_config(google_key="AIza-google"))
    settings = runner._build_fa_settings()
...
def test_generic_provider_in_settings():
    runner = _make_runner(
        pyclaw_config=_make_pyclaw_config(generic_key="mm-key", generic_url="https://api.minimax.io"),
    )
    settings = runner._build_fa_settings()
...
def test_generic_fallback_from_api_key_field():
    """Without pyclaw_config, generic provider falls back to self.api_key/base_url."""
    runner = _make_runner(
...
def test_unknown_server_skipped(caplog):
    import logging
    runner = _make_runner(
        servers=["pyclaw", "my-unknown-server"],
        pyclaw_config=_make_pyclaw_config(),
    )
    with caplog.at_level(logging.WARNING, logger="pyclaw.agents.runner"):
        settings = runner._build_fa_settings()
