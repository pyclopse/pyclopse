"""Global reflection registry for pyclaw.
...
def query(category: str | None = None, name: str | None = None) -> str:
    ...
    - no args          → module-level architecture overview (pyclaw.__doc__)
    ...
    if category is None and name is None:
        import pyclaw  # lazy — avoids circular import
        overview = inspect.getdoc(pyclaw) or "No overview available."
        return overview
...
def _query_config(section: str) -> str:
    from pyclaw.config import schema as cfg_schema  # lazy import
