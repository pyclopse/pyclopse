...
def _write_session_metadata(
    ...
    """Write the pyclaw session.json metadata file for an imported session.

    Counts user/assistant messages from the record list, builds the pyclaw
...
def import_agent_sessions(
    ...
    """Import all JSONL session files for a single agent from OpenClaw into pyclaw.
    ...
    ``pyclaw_dir/agents/{agent_name}/sessions/{session_id}/``.
    ...
    Args:
        ...
        pyclaw_dir (Path): Root pyclaw data directory (e.g. ``~/.pyclaw``).
...
def cmd_import_openclaw(args: Any) -> None:
    """Entry point for the ``pyclaw import-openclaw`` CLI command.

    Reads ``args.openclaw_dir``, ``args.pyclaw_dir``, ``args.agent``, and
    ...
    Args:
        args (Any): Parsed CLI arguments object with attributes:
            ...
            - ``pyclaw_dir`` (Optional[str]): Path to pyclaw data dir.
