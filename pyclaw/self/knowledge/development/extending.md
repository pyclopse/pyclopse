...
1. Create `pyclaw/channels/myplatform.py` implementing `ChannelPlugin`:
...
myplatform = "pyclaw.channels.myplatform:MyPlatformPlugin"
```
...
plugins:
  channels:
    - pyclaw.channels.myplatform:MyPlatformPlugin
```

3. Add a config class for the platform in `pyclaw/config/schema.py` and add it
   to `ChannelsConfig`.
...
All tools live in `pyclaw/tools/server.py`. Add a new `@mcp.tool()` function:
...
Register in `pyclaw/hooks/registry.py` after the registry is created, or in
a bundled hook file. For a bundled hook:

1. Create `pyclaw/hooks/bundled/my-hook/handler.py`:
...
1. Add a Pydantic model to `pyclaw/config/schema.py`:
...
1. Create `pyclaw/api/routes/my_thing.py`:
...
1. Create `pyclaw/providers/myprovider.py` implementing the provider interface.
