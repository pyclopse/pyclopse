...
**Files:** `pyclaw/channels/`, `pyclaw/channels/plugin.py`,
`pyclaw/channels/loader.py`
...
## ChannelPlugin ABC (`pyclaw/channels/plugin.py`)
...
## Telegram (`pyclaw/channels/telegram.py`)
...
## Slack (`pyclaw/channels/slack.py`)

### Threading

When `threading: true` in `SlackConfig`, each Slack thread becomes its own
pyclaw session. Session key: `thread_ts or ts` (the thread root timestamp).
...
1. **Entry points** — `pyclaw.channels` entry point group in `pyproject.toml`
