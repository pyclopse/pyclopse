...
**Files:** `pyclaw/core/agent.py`, `pyclaw/agents/runner.py`, `pyclaw/agents/factory.py`

---

## Agent Dataclass (`pyclaw/core/agent.py`)
...
## AgentManager (`pyclaw/core/agent.py`)
...
## AgentRunner (`pyclaw/agents/runner.py`)

`AgentRunner` wraps FastAgent for a single named agent instance. It is the
boundary between pyclaw's session model and FastAgent's execution engine.
...
runner = AgentRunner(
    ...
    owner_name="assistant",              # pyclaw agent id
    instruction=system_prompt,           # assembled system prompt
    model="sonnet",
    servers=["pyclaw", "fetch"],         # MCP servers to connect to
...
## AgentFactory (`pyclaw/agents/factory.py`)
...
The system prompt is assembled by `pyclaw/core/prompt_builder.py` from files
in `~/.pyclaw/agents/{agent_id}/`:
