...
```
1. pyclaw/skills/              bundled with package (lowest priority)
2. ~/.pyclaw/skills/           user-installed (managed)
...
SkillRegistry.lookup("daily-briefing")
         │
         ├─ load SKILL.md body
         ├─ replace {args} → "9am", {baseDir} → "~/.pyclaw/skills/daily-briefing"
         ▼
...
Agent (with full MCP tool access)
         ...
         ├─ runs: uv run ~/.pyclaw/skills/daily-briefing/scripts/setup.py --time "9am"
         ├─ reads script output
         ▼
Response to user
```
...
3. Run `uv run {baseDir}/scripts/init_skill.py <name> --path ~/.pyclaw/skills`
...
1. **`SkillRegistry`** class (`pyclaw/skills/registry.py`)
...
3. **Agent-scoped skills**: Should skills be definable per-agent in `pyclaw.yaml`
