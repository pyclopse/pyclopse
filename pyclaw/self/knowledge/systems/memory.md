...
**Files:** `pyclaw/memory/service.py`, `pyclaw/memory/file_backend.py`,
`pyclaw/memory/backend.py`, `pyclaw/memory/embeddings.py`,
`pyclaw/memory/clawvault.py`
...
## FileMemoryBackend (`pyclaw/memory/file_backend.py`)

### Daily Journals

Per-agent daily markdown files at:
```
~/.pyclaw/agents/{agent_id}/memory/YYYY-MM-DD.md
```
...
```
~/.pyclaw/agents/{agent_id}/memory/MEMORY.md
```
...
## Vector Search (`pyclaw/memory/embeddings.py`)
...
## ClawVault (`pyclaw/memory/clawvault.py`)
