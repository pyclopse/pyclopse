...
Jobs are persisted to `~/.pyclaw/agents/{agent_id}/jobs.yaml` (one file per agent) and survive restarts. Each run is appended to a per-job JSONL log under `~/.pyclaw/agents/{agent_id}/runs/`.
...
Each flag maps to a bootstrap file in `~/.pyclaw/agents/{agent}/`:
...
- **Job definitions**: `~/.pyclaw/agents/{agent_id}/jobs.yaml` — per-agent YAML, keyed by job name, atomic write
- **Run logs**: `~/.pyclaw/agents/{agent_id}/runs/{job_id}.jsonl` — one JSON line per completed run
...
Agents can manage jobs from within a conversation via the `job_*` MCP tools exposed by the pyclaw MCP server:
...
jobs:
  enabled: true
  agentsDir: ~/.pyclaw/agents       # where per-agent jobs.yaml files live
  defaultTimezone: America/New_York  # used when a cron job has no explicit tz
```

`defaultTimezone` should be set to your local timezone to avoid surprises with cron expressions. Without it, pyclaw falls back to the system local timezone.
...
The subagent system is a thin wrapper around the jobs system. When an agent calls `subagent_spawn`, pyclaw creates an ephemeral `AgentRun` job with:
