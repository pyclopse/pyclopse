"""DocLoader — reads pyclaw's self-knowledge base and source code.

Knowledge base lives at pyclaw/self/knowledge/ and is shipped as package data.
Source files are read from the pyclaw package directory (works in both dev and
...
logger = logging.getLogger("pyclaw.self")

# pyclaw/self/knowledge/
_KNOWLEDGE_DIR = Path(__file__).parent / "knowledge"

# pyclaw/ package root — one level up from pyclaw/self/
_PACKAGE_DIR = Path(__file__).parent.parent


class DocLoader:
    """Loads documentation topics and pyclaw source files.
    ...
    def source(self, module: str) -> str:
        """Read a pyclaw source file with line numbers.

        *module* is a path relative to the pyclaw package root, e.g.
        ``'core/gateway.py'`` or ``'agents/runner.py'``. Only paths within
        the pyclaw package are accessible; directory traversal is rejected.
        ...
        if not str(resolved).startswith(str(package_root)):
            return "[ERROR] Path escapes the pyclaw package directory."

        if not resolved.exists():
            return (
                f"[NOT FOUND] '{module}' not found in pyclaw package.\n"
    ...
    def _build_topic_index(self) -> str:
        ...
        lines = ["# pyclaw Self-Knowledge Topics", ""]
        for topic in topics:
