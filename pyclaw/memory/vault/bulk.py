...
Sessions
    ...
    ``~/.pyclaw/agents/{name}/sessions/*/history.json``.  Each file is a JSON
    object ``{"messages": [...]}`` where every message has ``role`` and
    ``content`` (string or list-of-content-parts).

Memory files
    Markdown files under ``~/.pyclaw/agents/{name}/memory/`` (daily journal
...
logger = logging.getLogger("pyclaw.vault.bulk")
