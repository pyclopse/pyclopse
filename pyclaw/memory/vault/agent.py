...
logger = logging.getLogger("pyclaw.memory.vault")
...
class FastAgentMemoryAgent(MemoryAgent):
    ...
    extraction call is stateless.  The model string is in pyclaw format
    ...
    Args:
        model: Model string in pyclaw format or FA format.
        pyclaw_config: Loaded pyclaw Config object (used for provider
    ...
    def __init__(
        ...
        self._pyclaw_config = pyclaw_config
    ...
    def _get_runner(self) -> Any:
        if self._runner is None:
            from pyclaw.core.agent import _get_provider_cfg, _translate_to_fa_model
            from pyclaw.agents.runner import AgentRunner

            fa_model = _translate_to_fa_model(self._raw_model, self._pyclaw_config)
            ...
            if "/" in self._raw_model:
                ...
                pcfg = _get_provider_cfg(self._pyclaw_config, provider_name)
                if pcfg:
            ...
            self._runner = AgentRunner(
                ...
                api_key=api_key,
                base_url=base_url,
                pyclaw_config=self._pyclaw_config,
