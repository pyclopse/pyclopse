...
logger = logging.getLogger("pyclopse.secrets")
...
class SecretsManager:
    """Registry-based secret resolution for pyclopse config.

    Secrets are **never** specified inline in config values.  Each secret is
    registered by name in ``~/.pyclopse/secrets/secrets.yaml`` (or in the
    ``secrets:`` block inside ``pyclopse.yaml`` as a fallback).  Anywhere in
