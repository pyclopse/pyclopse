...
from pyclopse.secrets.models import SecretsConfig
from pyclopse.memory.vault.config import VaultConfig as _VaultConfig
...
class DockerSandboxConfig(BaseModel):
    ...
    image: str = "pyclopse-sandbox:latest"
    network: str = "none"
...
class AuditConfig(BaseModel):
    ...
    enabled: bool = True
    log_file: str = "~/.pyclopse/logs/audit.log"
...
class AgentConfig(BaseModel):
    ...
    # Leave unset if unknown; pyclopse will estimate from history file size.
    ...
    # Skills here override same-named global skills. Searched after ~/.pyclopse/skills/
    # and ~/.pyclopse/agents/{name}/skills/ but before any gateway.skills_dirs paths.
...
class PluginType(str, Enum):
    """Types of plugins supported by pyclopse."""
...
class PluginsConfig(BaseModel):
    ...
    # Entry-point plugins (pyclopse.channels group) are always discovered
...
class ChromeDevtoolsMcpConfig(BaseModel):
    ...
    Then add "chrome-devtools" to your agent::
        ...
        agents:
          assistant:
            mcpServers: [pyclopse, fetch, chrome-devtools]
    """
    enabled: bool = False
...
class GatewayConfig(BaseModel):
    ...
    # Additional skill search directories (on top of ~/.pyclopse/skills/)
    skills_dirs: List[str] = Field(
        default_factory=list,
        validation_alias=AliasChoices("skills_dirs", "skillsDirs"),
    )
...
class Config(BaseModel):
    ...
    Loaded from ``~/.pyclopse/config.yaml`` (or ``--config`` path).  All top-level
    ...
    named secret registered in ``~/.pyclopse/secrets/secrets.yaml``.
    ...
    # All pyclopse timestamps use this zone.  Omit to use the system local timezone.
