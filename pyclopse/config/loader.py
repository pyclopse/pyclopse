...
from pyclopse.secrets.manager import SecretsManager

# Load .env file from common locations
for env_path in [
    Path.home() / ".pyclopse" / ".env",
    Path.home() / ".env",
    Path(".env"),
]:
    if env_path.exists():
...
# Dedicated secrets registry file — lives outside pyclopse.yaml so it can be
# managed independently (different permissions, separate git-ignore, etc.)
SECRETS_FILE_PATH = "~/.pyclopse/secrets/secrets.yaml"


DEFAULT_CONFIG_PATHS = [
    "~/.pyclopse/config/pyclopse.yaml",
    "~/.pyclopse/config.yaml",
    "~/.pyclopse/config.yml",
    "~/.pyclopse/pyclopse.yaml",
    "./config.yaml",
    "./config.yml",
    "./pyclopse.yaml",
]
...
def load_secrets_registry(config_path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
    ...
    Checks ``~/.pyclopse/secrets/secrets.yaml`` first.  If that file does not
    exist, falls back to the ``secrets:`` block inside the main config file
    (for users who have not yet migrated).

    Args:
        config_path: Path to the main pyclopse config file, used only for the
    ...
    if secrets_path.exists():
        try:
            ...
            import logging
            logging.getLogger("pyclopse.config").warning(
                f"Failed to load secrets file {secrets_path}: {e}"
            )

    # Fallback: secrets: block inside pyclopse.yaml
...
class ConfigLoader:
    """Loads and manages pyclopse configuration.
    ...
    def load(self, config_path: Optional[Union[str, Path]] = None) -> Config:
        ...
        # Registry is loaded from secrets.yaml (or pyclopse.yaml fallback).
...
def create_default_config(path: Union[str, Path] = "~/.pyclopse/config.yaml") -> Config:
    ...
    Args:
        path (Union[str, Path]): Destination path for the new config file.
            Defaults to ``~/.pyclopse/config.yaml``.
