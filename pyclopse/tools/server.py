"""
pyclopse MCP tool server.

Exposes pyclopse-native tools via the MCP protocol so FastAgent can use them.
Run as: uv run python -m pyclopse.tools.server
...
from pyclopse.reflect import reflect_system as _reflect_system


@_reflect_system("mcp-server")
class _PyclawMCPServer:
    """FastMCP tool server — exposes all pyclopse-native tools to FastAgent.
...
# Logs every tool call to the per-agent logger (pyclopse.agent.<name>).
# ---------------------------------------------------------------------------
...
class _ToolLoggingMiddleware(Middleware):
    ...
    async def on_call_tool(self, context: MiddlewareContext, call_next) -> Any:
        ...
        alogger = logging.getLogger(f"pyclopse.agent.{agent_name}")
        prefix = (
...
async def bash(
    ...
    # command's working directory rather than inheriting pyclopse's venv.
...
async def sessions_list() -> str:
    """List all active pyclopse sessions with their agent, channel, and status.
    ...
    try:
        ...
        loader = ConfigLoader()
        sessions_dir = Path("~/.pyclopse/sessions").expanduser()
...
def _agent_memory_service(agent_name: Optional[str] = None):
    ...
    Args:
        agent_name (Optional[str]): Agent name used to resolve the memory directory
            (``~/.pyclopse/agents/{agent_name}``).  Falls back to the
    ...
    config_dir = os.environ.get("PYCLAW_CONFIG_DIR", "~/.pyclopse")
    ...
    try:
        from pyclopse.memory.service import get_memory_service, MemoryService
...
async def session_status() -> str:
    ...
    try:
        import json
        status_file = Path("~/.pyclopse/status.json").expanduser()
...
async def sessions_send(
    ...
    """
    Send a message directly into another pyclopse agent's active session and get a response.
...
async def agents_list() -> str:
    """List all configured pyclopse agents with their model and status."""
    try:
        cfg_path = Path("~/.pyclopse/config/pyclopse.yaml").expanduser()
        if not cfg_path.exists():
            return "No pyclopse config found."
...
def _discover_all_skills(caller: Optional[str] = None) -> list:
    ...
    from pyclopse.skills.registry import discover_skills, get_skill_dirs, _parse_skill_dir
    import os as _os
    from pathlib import Path
    config_dir = Path(_os.path.expanduser("~/.pyclopse"))
...
def _find_config_path() -> Optional[Path]:
    """Locate the active pyclopse config file (same search order as ConfigLoader)."""
    from pyclopse.config.loader import DEFAULT_CONFIG_PATHS, expand_path
...
def config_get() -> str:
    """
    Return the current pyclopse configuration (sensitive fields redacted).
...
def config_set(path: str, value: str) -> str:
    ...
    try:
        ...
        if cfg_path is None:
            return "[ERROR] No pyclopse config file found."
    ...
    if cfg_path is not None:
        try:
            ...
            log_file = (
                raw.get("security", {})
                   .get("audit", {})
                   .get("log_file", "~/.pyclopse/logs/audit.log")
            )
            return Path(os.path.expanduser(log_file))
        except Exception:
            pass
    return Path(os.path.expanduser("~/.pyclopse/logs/audit.log"))
...
async def workflow_chain(
    ...
    try:
        from pyclopse.workflows.chain import run_chain
...
async def workflow_parallel(
    ...
    try:
        from pyclopse.workflows.parallel import run_parallel
...
async def secrets_list() -> str:
    """
    List all secret names registered in the pyclopse secrets registry.
...
async def secret_get(name: str) -> str:
    """
    Retrieve a named secret value from the pyclopse secrets registry.

    The secret must be registered under 'secrets:' in pyclopse.yaml.
...
def reflect(category: str = "", name: str = "") -> str:
    """Query pyclopse's live architecture reflection registry.

    Returns structured documentation about pyclopse's internals, derived from
    ...
    """
    from pyclopse.reflect import query as _reflect_query
...
def reflect_source(module: str) -> str:
    """Read pyclopse source code with line numbers.

    Returns the source of any file within the pyclopse package, with line numbers
    prepended to each line. Only paths within the pyclopse package are accessible.

    Use this for fine-grained implementation detail. Pair with reflect() for
    conceptual understanding.

    Args:
        module: Path relative to the pyclopse package root, using forward slashes.
    ...
    """
    from pyclopse.reflect import source_file
...
async def vault_search(query: str, limit: int = 15) -> str:
    ...
    try:
        from pyclopse.memory.vault.retrieval import infer_profile
...
async def vault_fact_store(
    ...
    try:
        from pyclopse.memory.vault.models import VaultFact, VaultFactState
        fact = VaultFact(
            claim=claim,
            type=type,
