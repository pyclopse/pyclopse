"""Import OpenClaw session history into pyclopse's FastAgent-native format.

Usage (from CLI):
    pyclopse import-openclaw [--agent NAME] [--all] [--openclaw-dir DIR]
...
Imported sessions land in:
    ~/.pyclopse/agents/{agent}/sessions/{YYYY-MM-DD}-{6chars}/
        session.json      (pyclopse metadata)
...
def _write_session_metadata(
    ...
    """Write the pyclopse session.json metadata file for an imported session.

    Counts user/assistant messages from the record list, builds the pyclopse
    ...
    Args:
        ...
        session_id (str): pyclopse session ID (e.g. ``2024-01-15-aB3xY7``).
...
def import_agent_sessions(
    ...
    """Import all JSONL session files for a single agent from OpenClaw into pyclopse.
    ...
    ``pyclopse_dir/agents/{agent_name}/sessions/{session_id}/``.
    ...
    Args:
        ...
        pyclopse_dir (Path): Root pyclopse data directory (e.g. ``~/.pyclopse``).
    ...
    sessions_dst_root = pyclopse_dir / "agents" / agent_name / "sessions"
    imported = 0
...
def cmd_import_openclaw(args: Any) -> None:
    """Entry point for the ``pyclopse import-openclaw`` CLI command.

    Reads ``args.openclaw_dir``, ``args.pyclopse_dir``, ``args.agent``, and
    ...
    Args:
        args (Any): Parsed CLI arguments object with attributes:
            ...
            - ``pyclopse_dir`` (Optional[str]): Path to pyclopse data dir.
    ...
    pyclopse_dir = Path(getattr(args, "pyclopse_dir", None) or _DEFAULT_PYCLAW_DIR).expanduser()
    ...
    if agent_name:
        ...
        print(
            ...
            "Example: pyclopse import-openclaw --all"
        )
        import sys; sys.exit(1)
    ...
    for name in agents_to_import:
        ...
        count = import_agent_sessions(name, openclaw_dir, pyclopse_dir)
        print(f"  → {count} session(s) imported")
        total += count

    print(f"\n✓ Done. {total} session(s) imported to {pyclopse_dir}/agents/")
    if total > 0:
        print("  Run 'pyclopse run' and the sessions will be available in the TUI session list.")
    print()
    print("⚠  Post-migration checklist:")
    print("   Review your agent markdown files for leftover OpenClaw paths:")
    print("   - PULSE.md  — update any script paths from ~/.openclaw/ to ~/.pyclopse/agents/{name}/")
    print("   - TOOLS.md  — update any tool/skill paths")
    print("   - scripts/  — update any hardcoded paths inside shell scripts or Python files")
    print("   Replace AGENTS.md with the pyclopse template (it still says 'OpenClaw' otherwise).")
