"""Global reflection registry for pyclopse.
...
def query(category: str | None = None, name: str | None = None) -> str:
    ...
    - no args          → module-level architecture overview (pyclopse.__doc__)
    ...
    if category is None and name is None:
        import pyclopse  # lazy — avoids circular import
        overview = inspect.getdoc(pyclopse) or "No overview available."
        return overview
...
def _query_config(section: str) -> str:
    from pyclopse.config import schema as cfg_schema  # lazy import
