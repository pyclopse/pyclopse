...
MCP tool queries it so agents can explore pyclopse's own architecture live.
...
class _ReflectionSystem:
    """Live architecture reflection system for pyclopse.

    Decorators annotate classes and functions at import time so agents can
    explore pyclopse's structure without reading static documentation.

    **Decorators:**

    - ``@reflect_system("name")`` — a major pyclopse subsystem (class or sentinel)
    ...
    **MCP tools (in pyclopse/tools/server.py):**
    ...
    **Query API:**
        ...
        reflect()                              → pyclopse.__doc__ overview
