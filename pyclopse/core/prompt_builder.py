...
# Files to read for system prompt - pyclopse names with OpenClaw aliases
BOOTSTRAP_FILES = [
    # pyclopse names (preferred)
    "AGENTS.md",     # Workspace instructions
    "PERSONALITY.md", # Agent personality (formerly SOUL.md)
    "IDENTITY.md",   # Agent identity
    "RULES.md",     # Operational rules (pyclopse addition)
    "USER.md",      # User information
...
def get_workspace_dir(config_dir: str = "~/.pyclopse") -> Path:
...
def build_system_prompt(
    ...
    lines = [
        "You are a personal assistant running inside pyclopse.",
        "",
    ...
    if not is_subagent:
        try:
            from pyclopse.skills.registry import discover_skills, format_for_prompt
...
def get_agent_file_path(agent_name: str, filename: str, config_dir: str = "~/.pyclopse") -> Optional[Path]:
...
def list_agent_files(agent_name: str, config_dir: str = "~/.pyclopse") -> list[str]:
...
def ensure_agent_files(agent_name: str, config_dir: str = "~/.pyclopse") -> dict[str, Path]:
