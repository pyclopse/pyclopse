...
1. **Entry points** — installed packages declare a ``pyclopse.channels`` entry
...
2. **Explicit config** — the ``plugins.channels`` list in ``pyclopse.yaml``
...
:class:`~pyclopse.channels.plugin.ChannelPlugin`.
"""
...
logger = logging.getLogger("pyclopse.channels")

_ENTRY_POINT_GROUP = "pyclopse.channels"
...
def discover_entry_points() -> List[ChannelPlugin]:
    ...
    ``pyclopse.channels`` will be loaded. Errors are logged and skipped so
