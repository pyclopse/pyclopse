...
logger = logging.getLogger("pyclopse.memory.vault")
...
class FastAgentMemoryAgent(MemoryAgent):
    ...
    extraction call is stateless.  The model string is in pyclopse format
    ...
    Args:
        model: Model string in pyclopse format or FA format.
        pyclopse_config: Loaded pyclopse Config object (used for provider
    ...
    def __init__(
        ...
        self._pyclopse_config = pyclopse_config
    ...
    def _get_runner(self) -> Any:
        if self._runner is None:
            from pyclopse.core.agent import _get_provider_cfg, _translate_to_fa_model
            from pyclopse.agents.runner import AgentRunner

            fa_model = _translate_to_fa_model(self._raw_model, self._pyclopse_config)
            ...
            if "/" in self._raw_model:
                ...
                pcfg = _get_provider_cfg(self._pyclopse_config, provider_name)
                if pcfg:
            ...
            self._runner = AgentRunner(
                ...
                api_key=api_key,
                base_url=base_url,
                pyclopse_config=self._pyclopse_config,
