...
Each agent gets its own directory under ``~/.pyclopse/agents/{agent_name}/``:
...
``pyclopse.core.prompt_builder.build_system_prompt``.
...
Vector index
------------
When an :class:`~pyclopse.memory.embeddings.EmbeddingBackend` is supplied, each
...
logger = logging.getLogger("pyclopse.memory")
    ...
    def __init__(
        ...
        Args:
            base_dir (str): The agent's root directory, e.g.
                ``~/.pyclopse/agents/myagent``.  The ``memory/`` subdirectory
