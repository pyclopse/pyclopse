...
# "pyclopse" provides all built-in tools: bash, memory, todos, sessions, etc.
_DEFAULT_SERVERS: List[str] = ["pyclopse"]

# Built-in server names that pyclopse knows how to configure programmatically.
_BUILTIN_SERVERS = frozenset(["pyclopse", "fetch", "time", "filesystem", "chrome-devtools"])
...
class AgentRunner:
    ...
    Attributes:
        ...
        pyclopse_config (Optional[Any]): Full Pyclopse config object for building FA settings.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            servers (Optional[List[str]]): MCP server names. Defaults to ``["pyclopse"]``.
            ...
            pyclopse_config (Optional[Any]): Full Pyclopse config object for building FA Settings. Defaults to None.
        """
        ...
        # Optional API key / base URL (from pyclopse.yaml providers.minimax)
        self.api_key = api_key
        self.base_url = base_url
        # The agent name sent as X-Agent-Name header to the pyclopse MCP server.
        ...
        # Full Pyclopse config — used to build FastAgent Settings programmatically
        self.pyclopse_config: Optional[Any] = pyclopse_config
    ...
    def _build_fa_settings(self) -> Any:
        """Build a FastAgent Settings object directly from pyclopse config.
        ...
        Pyclopse config object.  Unknown server names (not in ``_BUILTIN_SERVERS``)
        are skipped with a warning.

        Returns:
            Any: A constructed ``fast_agent.config.Settings`` instance with MCP
            server definitions and provider credentials populated from pyclopse config.
        """
        ...
        for name in (self._all_servers() or ["pyclopse"]):
            if name == "pyclopse":
                servers["pyclopse"] = MCPServerSettings(
                ...
                logger.warning(
                    ...
                    "define it under pyclopse config to use custom servers"
                )
        ...
        if cfg is not None:
            ...
            # No pyclopse_config — fall back to individual api_key/base_url fields
            provider_kwargs["generic"] = GenericSettings(
                api_key=self.api_key,
                base_url=self.base_url,
            )
    ...
    async def initialize(self) -> None:
        ...
        pyclopse config (replacing any on-disk ``fastagent.config.yaml``), registers
    ...
    async def run(self, prompt: str) -> str:
        ...
        _agent_logger = logging.getLogger(f"pyclopse.agent.{self.owner_name}")
        _p = self._log_prefix
        ...
        try:
            ...
            if self._slash_handler is None:
                ...
                self._slash_handler = SlashCommandHandler(
                    session_id=self.session_id or f"pyclopse-{self.agent_name}",
                    instance=instance,
