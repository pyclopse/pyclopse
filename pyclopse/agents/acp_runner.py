"""ACP (Agent Client Protocol) runners for pyclopse.
...
Provides three runner classes:
  ...
  AcpRunner        — Client-side stdio ACP runner.  Spawns any ACP-compliant
                     ...
                     drives it from pyclopse with full streaming support.
...
logger = logging.getLogger("pyclopse.agents.acp_runner")
...
class PyclawAcpClient:
    """
    Implements the ``acp.interfaces.Client`` protocol for pyclopse.
...
class AcpRunner:
    ...
    async def initialize(self) -> None:
        ...
        await conn.initialize(
            protocol_version=1,
            client_info=Implementation(name="pyclopse", version="1.0"),
