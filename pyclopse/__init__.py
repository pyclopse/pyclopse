"""pyclopse — modular AI agent gateway.

pyclopse connects LLM agents (backed by FastAgent) to messaging channels
...
```
1. load config
2. start pyclopse MCP server  (FastMCP on 8081) ← FastAgent connects here
...
|------------|------|----------------|---------------------------------------|
| pyclopse MCP | 8081 | FastMCP        | Tool server for FastAgent             |
...
| System       | File(s)                              | Role                                      |
|--------------|--------------------------------------|-------------------------------------------|
| gateway      | pyclopse/core/gateway.py               | Main orchestrator                         |
| agents       | pyclopse/core/agent.py                 | Agent + AgentManager                      |
| agent-runner | pyclopse/agents/runner.py              | FastAgent wrapper; streaming              |
| sessions     | pyclopse/core/session.py               | Metadata-only sessions + reaper           |
| commands     | pyclopse/core/commands.py              | /slash command dispatcher                 |
| queue        | pyclopse/core/queue.py                 | Per-session message queue (7 modes)       |
| jobs         | pyclopse/jobs/scheduler.py             | Cron/interval/one-shot job scheduler      |
| hooks        | pyclopse/hooks/                        | Event/hook registry + bundled hooks       |
| memory       | pyclopse/memory/                       | FileMemoryBackend + MemoryService routing |
| skills       | pyclopse/skills/                       | Skill discovery + injection               |
| channels     | pyclopse/channels/                     | Channel plugin system (Telegram, Slack)   |
| security     | pyclopse/security/                     | Exec approval + audit logging             |
| config       | pyclopse/config/                       | Pydantic schema + YAML loader             |
| mcp-server   | pyclopse/tools/server.py               | FastMCP tool server (port 8081)           |
| a2a          | pyclopse/agents/a2a.py                 | Agent-to-Agent protocol endpoints         |
| tui          | pyclopse/tui/                          | Textual TUI dashboard                     |
| concurrency  | pyclopse/core/concurrency.py           | Per-model asyncio semaphore throttling    |
| reflection   | pyclopse/reflect/                      | Live reflection of pyclopse architecture    |
...
Use the ``reflect`` MCP tool to explore pyclopse's architecture live:
