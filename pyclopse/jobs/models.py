    ...
    try:
        ...
        for raw in data.get("jobs", []):
            try:
                ...
                import logging
                logging.getLogger("pyclopse.jobs").warning(f"Skipping invalid job: {e}")
        return jobs
    except Exception as e:
        import logging
        logging.getLogger("pyclopse.jobs").error(f"Error loading jobs: {e}")
        return {}
...
def save_agent_jobs(jobs: Dict[str, Job], agent_dir: Path) -> None:
    """Write jobs to ``~/.pyclopse/agents/{id}/jobs.yaml`` keyed by job name.
    ...
    Args:
        jobs (Dict[str, Job]): Mapping of job ID to Job instance to persist.
        agent_dir (Path): Agent directory (e.g., ``~/.pyclopse/agents/assistant``).
    ...
    try:
        ...
        import logging
        logging.getLogger("pyclopse.jobs").error("ruamel.yaml not installed; cannot save agent jobs")
        return
...
def load_agent_jobs(agent_dir: Path) -> Dict[str, Job]:
    """Load jobs from ``~/.pyclopse/agents/{id}/jobs.yaml``.
    ...
    try:
        ...
        import logging
        logging.getLogger("pyclopse.jobs").error("ruamel.yaml not installed; cannot load agent jobs")
        return {}
    except Exception as e:
        import logging
        logging.getLogger("pyclopse.jobs").error(f"Error loading {path}: {e}")
        return {}
    ...
    for job_name, job_raw in raw_data.get("jobs", {}).items():
        ...
        try:
            ...
            import logging
            logging.getLogger("pyclopse.jobs").warning(f"Skipping invalid job {job_name!r}: {e}")
    return jobs
