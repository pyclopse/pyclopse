...
logger = logging.getLogger("pyclopse.api.config")
...
async def list_secrets():
    ...
    """
    from pyclopse.config.loader import load_secrets_registry
...
async def get_secret(name: str):
    """
    Resolve a named secret via the pyclopse secrets manager.

    The secret must be registered in ~/.pyclopse/secrets/secrets.yaml.
    """
    from pyclopse.config.loader import load_secrets_registry
